...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    //list *options = read_data_cfg(datacfg);
    //char *train_images = option_find_str(options, "train", "data/train.list");
    //char *backup_directory = option_find_str("/home/zh/darknet/backup");//option_find_str(options, "backup", "/backup/");
    ...
    list *plist = get_paths("/home/huluwa/darknet/2012_train.txt");//get_paths(train_images);
    //int N = plist->size;
    char **paths = (char **)list_to_array(plist);
...
#endif
            char buff[256];
            sprintf(buff, "%s/%s.backup", "/home/huluwa/darknet/backup"/*backup_directory*/, base);
            save_weights(net, buff);
        }
...
#endif
            char buff[256];
            sprintf(buff, "%s/%s_%d.weights", "/home/huluwa/darknet/backup"/*backup_directory*/, base, i);
            save_weights(net, buff);
        }
        free_data(train);
    }
...
#endif
    char buff[256];
    sprintf(buff, "%s/%s_final.weights", "/home/huluwa/darknet/backup"/*backup_directory*/, base);
    save_weights(net, buff);
}
